...
- **Owner:** `hooks-mode`
- **When:** A warn-listed tool is called. Fires once per decision: once
  when the tool is denied on the first call (`outcome="denied"`), and once
  when the same tool is subsequently allowed (`outcome="allowed"`).
  The `warned_tools` set is reset on every mode activation/clear, so
  each new mode activation starts the warn cycle fresh.
- **Payload:**
  ```python
  {
      "tool_name": str,
      "mode": str,
      "outcome": str,   # "denied" | "allowed"
  }
  ```
- **Not emitted when:** The tool is in the safe list, confirm list,
  block list, or bypassed as an infrastructure tool.
